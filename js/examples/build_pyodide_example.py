...
def main(output: Path, codemirror_wheel: Path, dependency_dir: Path) -> None:
    spaday_wheel = one_wheel(dependency_dir, "spaday")
    wheels = {
        "spaday": copy_wheel(spaday_wheel, output),
        "codemirror": copy_wheel(codemirror_wheel, output),
    }
    extract_assets(codemirror_wheel, "spaday_codemirror", output / "components" / "codemirror")
