...
catalog:  ## regenerate the typed component from its manifest
	python -m spaday.cem spaday_codemirror/components.cem.json -o spaday_codemirror/components.py
	python -m ruff format spaday_codemirror/components.py
...
lint-py:  ## run python linter with ruff
	python -m ruff check spaday_codemirror
	python -m ruff format --check spaday_codemirror
...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix spaday_codemirror
	python -m ruff format spaday_codemirror
...
test-py:  ## run python tests
	python -m pytest -v spaday_codemirror/tests

# alias
tests-py: test-py

coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v spaday_codemirror/tests --cov=spaday_codemirror --cov-report term-missing --cov-report xml
...
pyodide-example: build  ## build the standalone Pyodide example into dist/lite
	...
	python -m pip download --no-deps --only-binary=:all: --platform pyemscripten_2026_0_wasm32 --python-version 314 --implementation cp --abi cp314 --dest dist/pyodide-deps "spaday==0.11.2"
...
test-pyodide-example: pyodide-example  ## run the standalone Pyodide example in Chromium
	...
	cd js; SPADAY_CODEMIRROR_PYODIDE_ONLY=1 pnpm exec playwright test tests/pyodide.spec.js
